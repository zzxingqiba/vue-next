...
export function getSequence(arr) {
  ...
  for (let i = 0; i < sourceLength; i++) {
    ...
    while (left < right) { // 结束标准: start == end    当前值current  索引i   目标值result[end]
      let middle = ((left + right) / 2) | 0   // 首次来看长度为9/2 | 0 索引为4  取值为6
      if (arr[result[middle]] < current) { // result[middle]取值为递增序列的中间值  我们要取到arr中所对应的位置作比较 arr[result[middle]]
        left = middle + 1
      } else {
        right = middle
      }
